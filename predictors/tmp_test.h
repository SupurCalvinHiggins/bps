...
#ifndef META_TABLE_BITS
#define META_TABLE_BITS (12)
#endif

#define META_TABLE_LEN (1 << META_TABLE_BITS)
...
#define SIZE                                                                   \
  (GSHARE_COUNTER_BITS * GSHARE_TABLE_LEN + 2 * META_TABLE_LEN +               \
...
public:
  PREDICTOR(void)
      : m_gshare_table(GSHARE_TABLE_BITS, 0),
        ...
        m_history_hash(HISTORY_COUNT, 0), m_meta(META_TABLE_LEN, 2) {
  ...
  bool GetPrediction(u32 PC) {
    ...
    if (m_primary && m_alt) {
      ...
      // selection we use PC mod META_TABLE_LEN (simple)
      usize midx = (PC ^ (m_primary_idx * 0x9e3779b9u)) & (META_TABLE_LEN - 1);
      u8 meta_ctr =
