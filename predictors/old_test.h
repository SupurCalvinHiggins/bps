...
// corrected apow
constexpr f64 apow(const f64 base, int power) {
  assert(power >= 0);
  assert(base > 1.0);
  f64 result = 1.0;
  while (power--)
...
public:
  PREDICTOR(void)
      : m_gshare_table(GSHARE_PATTERN_TABLE_BITS, 0),
        ...
        m_history(0), m_history_hash(TAGE_HISTORY_COUNT, 0),
        m_meta(META_TABLE_SIZE, 2) {
    ...
    m_history = History(m_history_bits.back());
