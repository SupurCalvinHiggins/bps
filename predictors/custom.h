...
// -------------------------------------------------------------
// Configurable parameters: tune these to trade area/accuracy.
// -------------------------------------------------------------
...
// counter bits used in gshare table (we'll use 3-bit everywhere)
...
#define META_TABLE_SIZE (1 << META_TABLE_BITS)

// Decay period (number of branches between usefulness decay)
#define UBIT_DECAY_PERIOD (1024) // tune (e.g., 1024)

// Allocation probing attempts
...
class PREDICTOR {
  ...
  // History ring buffer (explicit 0/1 bytes, O(1) update)
  std::vector<u8> m_history;
  usize m_history_head = 0;
...
#define USED_GSHARE 2
  ...
  // meta predictor: 2-bit saturating counters (0..3). >=2 favors primary.
  std::vector<u8> m_meta;

  // for usefulness decay
  usize m_branch_since_decay = 0;

  // hashing state/ordering
  void set_hashes(u32 PC) {
    // progressive hashing: PC then newer-to-older history bits progressively
    XXH32_state_t *state = XXH32_createState();
    XXH32_reset(state, 0);
    XXH32_update(state, &PC, 4);

    // feed history bits newest-first
    for (usize i = 0; i < m_history_bits.size(); ++i) {
      auto bits = m_history_bits[i];
      // feed additional bits required (we will reinitialize for each table?
      // original code progressive)
      for (usize b = 0; b < bits; ++b) {
        // b-th newest bit: history_head - b (mod size)
        usize idx = (m_history_head + m_history.size() - b) % m_history.size();
        u8 bit = m_history[idx];
        XXH32_update(state, &bit, 1);
      }
      m_history_hashes[i] = XXH32_digest(state);
      // For progressive behavior, we could avoid resetting state and simply
      // continue feeding more bits. But the above re-hashes PC+full-prefix for
      // each table; it is fine for now.
    }
...
public:
  PREDICTOR(void)
      : m_gshare_table(GSHARE_PATTERN_TABLE_BITS, 0),
        ...
        m_meta(META_TABLE_SIZE,
               2) // initialize meta counters to weakly favor primary (2)
  {
    // build geometric history lengths
    m_history_bits.reserve(TAGE_HISTORY_COUNT);
    for (usize i = 0; i < TAGE_HISTORY_COUNT; ++i) {
      // ensure integer history bits
      m_history_bits.push_back(static_cast<usize>(std::max<size_t>(
          TAGE_MIN_HISTORY_LEN,
          static_cast<size_t>(
              round(TAGE_MIN_HISTORY_LEN * apow(TAGE_HISTORY_GROWTH, i))))));
    }

    // history ring buffer allocate to longest history
    m_history.reserve(m_history_bits.back());
    m_history.resize(m_history_bits.back(), 0);
    m_history_head = 0;
  ...
  bool GetPrediction(u32 PC) {
    ...
    if (m_primary && m_alt) {
      // meta index derived from PC and primary index to get PC-specific
      // selection we use PC mod META_TABLE_SIZE (simple)
      usize midx = (PC ^ (m_primary_idx * 0x9e3779b9u)) & (META_TABLE_SIZE - 1);
      u8 meta_ctr =
          m_meta[midx]; // 0..3, >=2 favors primary, <2 favors alternate
      bool choose_primary = meta_ctr >= 2;
      if (choose_primary) {
        m_used = USED_PRIMARY;
        return m_primary->prediction();
      } else {
  ...
  void UpdatePredictor(u32 PC, bool resolveDir, bool predDir,
    ...
    // Update meta predictor based on the pre-update predictions (NOT
    // post-update!)
    if (m_primary && m_alt && (prim_pred != alt_pred)) {
      usize midx = (PC ^ (m_primary_idx * 0x9e3779b9u)) & (META_TABLE_SIZE - 1);
      // primary correct & alt wrong -> favor primary
      if (prim_pred == resolveDir && alt_pred != resolveDir) {
        if (m_meta[midx] < 3)
          ++m_meta[midx];
      } else if (prim_pred != resolveDir && alt_pred == resolveDir) {
        if (m_meta[midx] > 0)
          --m_meta[midx];
      }
    }
    ...
    m_history_head = (m_history_head + 1) % m_history.size();
    m_history[m_history_head] = resolveDir ? 1 : 0;
  };
