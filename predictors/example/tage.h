...
// #define SHARINGTABLES		// let us share the physical among
// several logic predictor tables #define INITHISTLENGTH		// uses
// the "best" history length we found
...
// #define LOGB 15			// log of number of entries in bimodal
// predictor
#define HYSTSHIFT                                                              \
  2 // sharing an hysteris bit between 4 bimodal predictor entries
// #define LOGG (LOGB-4)		// initial definition was with 2K
// entries per tagged tables
...
#define TBITS 16    // was 6
#define MAXTBITS 31 // was 15
#endif

// #define CWIDTH 3		// predictor counter width on the tagged tables
#define HISTBUFFERLENGTH                                                       \
  131072 // we use a 4K entries history buffer to store the branch history

// Statistical corrector parameters
#define NSTAT 9 // was 5
#define CSTAT 6
// #define LOGStatCor (LOGG+1)	// the Statistical Corrector predictor features
// 5 * 1K entries
#define LOGStatCor (18) // was 12
...
#define LOGL 18 // was 6: 64 entries loop predictor 4-way skewed associative
#define WIDTHNBITERLOOP                                                        \
  15               // was 10: we predict only loops with less than 1K iterations
#define LOOPTAG 15 // was 10: tag width in the loop predictor
#define LOGSPEC 6
/////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////
...
public:
  ...
  uint32_t CurrentIterSpec; // 10 bits
...
private:
  ...
  int8_t USE_ALT_ON_NA; // "Use alternate prediction on newly allocated":  a
                        // 4-bit counter  to determine whether the newly
                        // allocated entries should be considered as  valid or
                        // not for delivering  the prediction
  int TICK,
      LOGTICK; // control counter for the smooth resetting of useful counters
  ...
  // folded_history Fetch_ch_i[NHIST + 1];	//utility for computing TAGE
  // indices folded_history Fetch_ch_t[2][NHIST + 1];	//utility for computing
  // TAGE tags
  ...
  // folded_history Retire_ch_i[NHIST + 1];	//utility for computing TAGE
  // indices folded_history Retire_ch_t[2][NHIST + 1];	//utility for computing
  // TAGE tags
  ...
  uint32_t LTAG;   // tag on the loop predictor
  ...
  // int logg[NHIST + 1];		// log of number entries of the
  // different tagged tables

  // int GI[NHIST + 1];		// indexes to the different tables are computed
  // only once int GTAG[NHIST + 1];		// tags for the different tables
  ...
  uint32_t *GTAG;  // tags for the different tables are computed only once
...
public:
  ...
  tage(int counter, int numtable, int tablesize, int min, int max) {
    ...
    */
    ...
    for (int i = 4; i <= 7; i++)
      if (NHIST >= i)
        logg[i] = LOGG + 2; // was LOGG + 1
    for (int i = 8; i <= 11; i++)
      if (NHIST >= i)
        logg[i] = LOGG + 1; // was LOGG
    for (int i = 12; i <= NHIST; i++)
      if (NHIST >= i)
        logg[i] = LOGG; // was LOGG -1
    for (int i = 1; i <= NHIST; i++) {
...
#define PRINTCHAR
    ...
    NBENTRY += (1 << (logg[1])) + (1 << (logg[STEP1])) + (1 << (logg[STEP2])) +
               (1 << (logg[STEP3]));
#endif
    ...
    // Entry width on the speculative table is 20 bits (19 bits to identify the
    // table entry that gives the prediction and 1 prediction bit
#endif
    fprintf(stdout, "TOTAL STORAGESIZE= %d bytes\n", STORAGESIZE / 8);
#endif
  }

  int bindex(uint32_t pc, int size) { return ((pc) & ((1 << (size)) - 1)); }
  ...
  //  tag computation
  uint32_t gtag(unsigned int pc, int bank, folded_history *ch0,
                folded_history *ch1) {
  ...
  bool getbim() { return (btable[BI].pred > 0); }
  // update  the bimodal predictor: a hysteresis bit is shared among 4
  // prediction bits
...
#ifdef LOOPPREDICTOR
  int lindex(uint32_t pc) { return ((pc & ((1 << (LOGL - 2)) - 1)) << 2); }
  ...
  bool GetPrediction(UINT32 PC) {
    ...
    for (int i = 1; i <= NHIST; i++) {
      ...
      GTAG[i] = gtag(pc, i, Fetch_ch_t1, Fetch_ch_t2);
    }
#ifdef SHARINGTABLES
    for (int i = 2; i <= STEP1 - 1; i++)
  ...
  //  UPDATE  FETCH HISTORIES   + spec update of the loop predictor + Update of
  //  the IUM
  ...
  void HistoryUpdate(uint32_t pc, OpType opType, bool taken, uint32_t target,
                     int &X, int &Y, folded_history *H, folded_history *G,
                     folded_history *J) {
    // special treatment for indirects and returnd: inhereited from the indirect
    // branch predictor submission
    ...
    for (int t = 0; t < maxt; t++) {
      ...
      // update  history
      Y--;
      ghist[Y & (HISTBUFFERLENGTH - 1)] = bTAKEN;
      X = (X << 1) + PATHBIT;
      X = (X & ((1 << PHISTWIDTH) - 1));
  ...
  void UpdatePredictor(UINT32 PC, bool resolveDir, bool predDir,
                       UINT32 branchTarget, int loopcount) {
    // burak
    uint32_t pc = PC & 0xffffff;
    ...
    for (int myn = 0; myn <= loopcount; myn++) {
      ...
      if (myn == 0) {
        ...
        // Note that on a real hardware processor, one would avoid this
        // recomputation to save an extra read port on the branch predictor. One
        // would  have to propagate informations needed for update with the
        // instruction: HitBank, updated through the STATCOR or not, etc.

        // Recompute the prediction (the indices and tags) with the Retire
        // history.
        for (int i = 1; i <= NHIST; i++) {
          GI[i] = gindex(pc, i, Retire_phist, Retire_ch_i);

          GTAG[i] = gtag(pc, i, Retire_ch_t1, Retire_ch_t2);
        }
#ifdef SHARINGTABLES
        for (int i = 2; i <= STEP1 - 1; i++)
          GI[i] = ((GI[1] & 7) ^ (i - 1)) + (GI[i] << 3);
        ...
        loopupdate(pc, taken, (tage_pred != taken)); // update the loop
                                                     // predictor
#endif
...
public:
  ...
  PREDICTOR(void) {
    ...
    //    int tsize[] =   {25, 25, 25, 25, 25, 25, 25, 25, 25, 25, 25, 25, 25,
    //    25, 25, 25, 25, 25, 25, 25, 25, 25, 25, 25, 25, 25, 25, 25, 25, 25,
    //    25, 25};

    //    int tsize[] =   {23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23,
    //    23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23,
    //    23, 23};
    // VERSION1: all 15 tables, same size, same counter width, just hist lengths
    // change
    /**/
    int cwidth[] = {3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3,
                    3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3};
    int ntables[] = {15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15,
                     15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15,
                     15, 15, 15, 15, 15, 15, 15, 15, 15, 15};
    int tsize[] = {21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21,
                   21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21,
                   21, 21, 21, 21, 21, 21, 21, 21, 21, 21};
    int minhist[] = {6,  7,  9, 7, 5, 6, 9, 10, 5, 4, 6, 8, 3, 4, 9,  7,
                     12, 11, 5, 7, 8, 6, 4, 3,  3, 4, 9, 8, 5, 3, 10, 9};
    double maxhist[] = {10,   9,   8,    7,  6,   5,  4,  3,  2.5, 2.25, 2,
                        1.75, 1.5, 1.25, 1,  .75, 10, 8,  6,  4,   2,    4.5,
                        3.5,  5.5, .3,   .4, .5,  .6, .7, .8, .9,  1}; // in K
                                                                       /**/

    // VERSION2: same as version1 except for cwidths are 3,4 and 5
    /*
    int cwidth[] =  {3,  4,  5,  4,  3,  5,  3,  4,  5,  4,  3,  5,  3,  4,  5,
    4,  3,  5,  3,  4,  5,  4,  3,  5,  3,  4,  5,  4,  3,  5,  3,  4}; int
    ntables[] = {15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15,
    15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15}; int tsize[]
    =   {21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21,
    21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21}; int minhist[] = {6,
    7,  9,  7,  5,  6,  9,  10, 5,  4,  6,  8,  3,  4,  9,  7,  12, 11, 5,  7,
    8,  6,  4,  3,  3,  4,  9,  8,  5,  3,  10, 9}; double maxhist[] = {10, 9,
    8,  7,  6,  5,  4,  3, 2.5,2.25,2, 1.75,1.5,1.25,1,.75, 10, 8,  6,  4,
    2, 4.5, 3.5,5.5,.3, .4, .5, .6, .7, .8, .9, 1}; // in K
    */

    // VERSION3: same as version 1 but more long histories, min short hist 5
    /**
    int cwidth[] =  {3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3,
    3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3}; int
    ntables[] = {15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15,
    15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15}; int tsize[]
    =   {21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21,
    21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21, 21}; int minhist[] = {6,
    7,  9,  7,  5,  6,  9,  10, 5,  9,  6,  8,  7,  9,  9,  7,  12, 11, 5,  7,
    8,  6,  9,  8,  7,  6,  9,  8,  5,  7,  10, 9}; double maxhist[] = {10, 9,
    8,  7,  6,  5,  4,  3, 2.5,2.25,2, 1.75,1.5,1.25,1,.75, 10, 8,  6,  4,
    2, 4.5, 3.5,5.5,2.3, 3.4, 4.5, 5.6, 15.7, 20.8, 8.9, 43.1}; // in K
    **/
    // VERSION4: same as version 3 but 4x tables
    /*
    int cwidth[] =  {3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3,
    3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3,  3}; int
    ntables[] = {15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15,
    ...
    23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23, 23}; int minhist[] = {6,
    7,  9,  7,  5,  6,  9,  10, 5,  9,  6,  8,  7,  9,  9,  7,  12, 11, 5,  7,
    8,  6,  9,  8,  7,  6,  9,  8,  5,  7,  10, 9}; double maxhist[] = {10, 9,
    8,  7,  6,  5,  4,  3, 2.5,2.25,2, 1.75,1.5,1.25,1,.75, 10, 8,  6,  4,
    2, 4.5, 3.5,5.5,2.3, 3.4, 4.5, 5.6, 15.7, 20.8, 8.9, 43.1}; // in K
    */
    ...
    for (int i = 0; i < tagecount; i++)
      // brpred[i] = new tage(cwidth[i], ntables[i], tsize[i], minhist[i],
      // int(maxhist[i]*1000));
      brpred[i] = new tage(cwidth[10], ntables[10], (tsize[10] + 5),
                           minhist[10], int(maxhist[10] * 1000));
  }
  ...
  bool GetPrediction(UINT32 PC) {
    ...
    for (int i = 0; i < tagecount; i++) {
      ...
      if (i > 0 && flag == 0) {
        flag = (prevpred != predDira[i]);
      }
      prevpred = predDira[i];

      weight = (predDira[i] == 0) ? (oobe[i] - oobe_hist_length)
                                  : (oobe_hist_length - oobe[i]);

      // weight = (predDira[i]==0)?-1:1;
    ...
    // char mych = ((flag == 1)?'F':'N');
    // cout << " -- " << predDirint << " -- " << mych << '\t' << (predDirint
    // >=0);
    return predDirint >= 0;
  }
  void UpdatePredictor(UINT32 PC, bool resolveDir, bool predDir,
    ...
    for (int i = 0; i < tagecount; i++) {
      ...
      //		  if(k==0){
      oobe[i] = oobe[i] -
                oobe_hist[i][(oobe_hist_inpoint[i] + oobe_hist_length - 1) %
                             oobe_hist_length] +
                (predDira[i] != resolveDir);
      ...
      if (oobe_hist_inpoint[i] == 0)
        ...
        oobe_hist_inpoint[i]--;
      //}
      brpred[i]->UpdatePredictor(PC, resolveDir, predDir, branchTarget,
                                 ((i == 0) ? 1 : k));
    }
    // cout << "real: " << resolveDir << endl;
  }
